...
object Validator {
    ...
    // is text input edit text Valid
    fun isCibValidText(cib: TextInputEditText, message: String, cill: TextInputLayout): Boolean {
        var isCibValidText = false
        try {
            if (cib.text.toString().trim().isEmpty()) {
                cill.error = message
                isCibValidText = false
            } else {
                cill.error = ""
                isCibValidText = true
            }
        ...
        return isCibValidText
    }
    fun isCibValidEditText(cib: EditText, message: String, cill: TextInputLayout): Boolean {
        var isCibValidText = false
        try {
            if (cib.text.toString().trim().isEmpty()) {
                cill.error = message
